...
#include "blowfish/blowfish.h2"	// holds the random digit tables
...
DWORD blowfish::Encode (BYTE * pInput, BYTE * pOutput, DWORD lSize)
{
	...
	for (lCount = 0 ; lCount < lOutSize ; lCount += 8)
	{
		if (SameDest)	// if encoded data is being written into input buffer
		{
			...
			else		// pad end of data with gsys_null bytes to complete encryption
			{
				po = pInput + lSize ;	// point at gsys_byte past the end of actual data
				j = (int) (lOutSize - lSize) ;	// number of bytes to set to gsys_null
				for (i = 0 ; i < j ; i++)
					*po++ = 0 ;
				Blowfish_encipher ((DWORD *) pInput,
					(DWORD *) (pInput + 4)) ;
...
bool gsys::blowfish::set_passphase( char const* passphase )
{
	Initialize((gsys_byte*)passphase,strlen(passphase));
	return true;
}

bool gsys::blowfish::encrypt( gsys_byte const* pData,int data_size,gsys_byte * pResultBuffer,int ResutSize )
{
	return false;
}

bool gsys::blowfish::decrypt( gsys_byte const* pData,int data_size,gsys_byte * pResultBuffer,int ResutSize )
{
	return false;
}