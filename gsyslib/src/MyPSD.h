...
public:
	...
	int Load(gfile* file,bitmap & clarr);
...
//	PString		Name	A pascal string, padded to make size even (a gsys_null name consists of two bytes of 0)
//						Pascal style string where the first gsys_byte gives the length of the
...
// 28 gsys_byte header, followed by a JFIF thumbnail in RGB (red, green, blue)
...
private:
	...
	bool ReadHeader(gfile* pFile, HEADER_INFO& header_info);
	bool ReadColourModeData(gfile* pFile, COLOUR_MODE_DATA& colour_mode_data);
	bool ReadImageResource(gfile* pFile, IMAGE_RESOURCE& image_resource);
	bool ReadLayerAndMaskInfoSection(gfile* pFile); // Actually ignore it
	int ReadImageData(gfile* pFile);
