...
typedef struct _tagDispOption
{
	...
	wchar_t value_quotation_mark;	// val="" (default value quotation mark "
	LPXENTITYS	entitys;	// entity table for entity encode

	wchar_t const* tab_chars;

	int tab_base;			// internal usage
	_tagDispOption() { newline = true; reference_value = true; entitys = &entityDefault; tab_base = 0; value_quotation_mark = L'"';tab_chars = L"\t"; }
}DISP_OPT, *LPDISP_OPT;
...
typedef struct _tagXMLNode
{
	...
	LPXNode operator [] ( LPCTSTR name ) { return Find(name); }
...
{
	int n = gsys::wchar_classify::length(str);

	if( !str || !str[0]  )return true;

	for(int i=0;i<n;i++)
	{
		if(!gsys::wchar_classify::whitespace(!str[i]))return false;
	}
	return true;
}
...
template <class T>
T xml_cast(dcmapWCSTR str)
{
	if(!str)return 0;
	T r;
	if(!lex_convert(r,str))return 0;
	return r;
}

template <>
int inline xml_cast<int>(dcmapWCSTR str)
{
	if(!str)return 0;
	return _wtoi(str);
}
