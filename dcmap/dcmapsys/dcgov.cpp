...
static int orig_govs_placement[MAX_GOVS];
static int govs_count = 0;
...
bool dcgovReadPlanets(IDCMapPlanetDataStorage* ipl, dcmapDBREF const& player)
{
	...
	IDCMapPlanetDataStoragePtr p(ipl);

	bool has_hw = false;

	planet_info_t* govs_planets[MAX_GOVS];

	planets_count=1;
	if(p->SelectAll())
	do 
	{
		if(p[DCMDB_PLANET_OWNER] == player)
		{
			memset(&info, 0, sizeof(planet_info_t));
			info.x = p[DCMDB_PLANET_X];
			info.y = p[DCMDB_PLANET_Y];
			info.hw = (bool)p[DCMDB_PLANET_HW];
			info.safe = !(bool)p[DCMDB_PLANET_UNSAFE];

			info.full_income = 0;
			info.max_corrupt = 100;

			int pnum = planets_count;
			if(info.hw)
			{
				if(has_hw)return false; // multiple hw present
				has_hw=true;
				pnum=0;
			}
			else planets_count++;

			planets[pnum] = (planet_info_t *)malloc(sizeof(planet_info_t));
			*planets[pnum] = info;

			int govs = p[DCMDB_PLANET_HAS_GOVERNORS];
			for(int i=0;i<govs;i++)
			{
				govs_planets[govs_count++] = planets[pnum];
			}
		}
	} while(p->Next());

	if(!has_hw)return false; // no hw present

	sort_planets_by_coords();
	calc_planet_distances();
	sort_planets_by_neighbs_count();

	for(int i=0;i<govs_count;i++)
	{
		for(int n=0;n<planets_count;n++)
		{
			if(govs_planets[i]==planets[n])
			{
				orig_govs_placement[i] = n;
				break;
			}
		}
	}
	sort_govs(govs_count, orig_govs_placement);            
	calc_total_income(govs_count,orig_govs_placement);
	return true;
}

bool dcgovUpdateCorruption(IDCMapPlanetDataStorage* ipl)
{
	IDCMapPlanetDataStoragePtr p(ipl);
	...
	for(int i=0;i<planets_count;i++)
	{
		...
		if(info && p->SelectCoord(info->x,info->y))
		{
			int corrupt = corrupts[info->corrupt];
			if((int)p[DCMDB_PLANET_CORRUPTION] != corrupt)
			{
				if(!upd)p->BeginUpdate();
				upd=true;
				p->Update();
				p[DCMDB_PLANET_CORRUPTION] = corrupt;
				p->Post();
			}
		}
	}
