...
void _SetString( LPTSTR psz, LPTSTR end, CString* ps, bool trim = FALSE, int escape = 0 )
{
	...
	if( escape )
	{
		...
		LPTSTR pss = ps->update_buffer( len+1 );
		_tcsecpy( pss, escape, psz, end );
	}
	else
	{
		LPTSTR pss = ps->update_buffer(len + 1 );
		memcpy( pss, psz, len*sizeof(dcmapWCHAR) );
		pss[len] = L'\0';
	}
}
		...
		if( xml = _tcsskip( xml ) )
		{
			...
			if( pEnd == NULL ) 
			{
				...
				if( pi->erorr_occur == false ) 
				{
					...
					pi->error_string =  str(wformat(_T("<%s> attribute has error "))%name);
				}
				return NULL;
			}
			...
			if( XIsEmptyString( value.c_str() ) )
			{
				...
				if( pEnd == NULL ) 
				{
					if( pi->erorr_occur == false ) 
					{
						...
						pi->error_string =  str(wformat(_T("%s must be closed with </%s>"))%name);
					}
						...
						else
						{
							...
							if( pi->force_parse == false )
							{
								...
								if( pi->erorr_occur == false ) 
								{
									...
									pi->error_string =  str(wformat(_T("'<%s> ... </%s>' is not wel-formed."))%name%closename);
								}
								return NULL;
							}
						}
...
CString _tagXMLAttr::GetXML( LPDISP_OPT opt /*= &optDefault*/ )
{
	...
	os << name << L"=" << (wchar_t)opt->value_quotation_mark 
		<< (opt->reference_value&&opt->entitys?opt->entitys->Entity2Ref(value.c_str()):value) 
		<< (wchar_t)opt->value_quotation_mark << L" ";
	return os.str().c_str();
}
...
LPCTSTR	_tagXMLNode::GetAttrStr( LPCTSTR attrname )
{
	LPXAttr attr = GetAttr( attrname );
	return attr ? attr->value.c_str() : L"";
}
...
// bro      2002-10-29
//========================================================
LPCTSTR	_tagXMLNode::GetChildValue( int i )
{
	LPXNode node = GetChild( i );
...
CString _tagXMLEntitys::Ref2Entity( LPCTSTR estr )
{
	...
	if( estr )
	{
		...
		LPTSTR esbuf = es.update_buffer( len+1 );
		if( esbuf )
...
CString _tagXMLEntitys::Entity2Ref( LPCTSTR str )
{
	...
	if( str )
	{
		...
		LPTSTR sbuf = s.update_buffer( len+1 );
		if( sbuf )
